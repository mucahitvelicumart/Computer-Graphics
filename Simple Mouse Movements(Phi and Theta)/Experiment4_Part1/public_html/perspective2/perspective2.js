...
window.onload = function init() {
    ...
    document.addEventListener('pointerlockchange', mouseEventForThetaAndPhi, false);
    document.addEventListener('mozpointerlockchange', mouseEventForThetaAndPhi, false);
    var control = 1;
    function mouseEventForThetaAndPhi() {
        if (document.pointerLockElement === canvas ||
                document.mozPointerLockElement === canvas) {
            document.addEventListener("mousemove", movement, false);

            function movement(event) {
                if (control === 0) {

                    theta += event.movementX / 1000.0;
                    phi += event.movementY / 1000.0;
                }
            }
        } else {
